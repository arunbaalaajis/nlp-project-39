# -*- coding: utf-8 -*-
"""
Created on Fri Dec  2 00:14:41 2022
...
class Tucker_Model(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        self.Tucker_Fusion = Tucker([config.hidden,config.hidden],config.hidden,mm_dim=1200,shared=False,normalize=False,dropout_input=0.0,dropout_pre_lin=0.0,dropout_output=0.0)
    ...
    def forward(self, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, return_yp=False):
        ...
        output = self.Tucker_Fusion([output,output_t])
...
class MFB_Model(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        self.MFB_Fusion = MFB([config.hidden,config.hidden],config.hidden,mm_dim=120,factor=2,activ_input='relu',activ_output='relu',normalize=False,dropout_input=0.0,dropout_pre_norm=0.0,dropout_output=0.0)
    ...
    def forward(self, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, return_yp=False):
        ...
        output = self.MFB_Fusion([output,output_t])
...
class MultiFuse_Model(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        super().__init__()
        self.config = config
        self.word_dim = config.glove_dim
    ...
    def forward(self, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, return_yp=False):
        ...
        output_mfb = self.MFB_Fusion([output,output_t])
        output_tucker = self.Tucker_Fusion([output,output_t])
        output = output_mfb+output_tucker
        sp_output = self.rnn_sp(output, context_lens)
