# -*- coding: utf-8 -*-
"""
Created on Fri Dec  2 00:56:23 2022
...
from Fusion_model import Model,MFB_Model,Tucker_Model,MultiFuse_Model #, NoCharModel, NoSelfModel
...
def train(config):
    ...
    if config.sp_lambda > 0:
        ...
        elif config.fusion =='MFB':
            model = MFB_Model(config, word_mat, char_mat)
        elif config.fusion =='Tucker':
            model = Tucker_Model(config, word_mat, char_mat)
        elif config.fusion == 'Multi':
            model = MultiFuse_Model(config, word_mat, char_mat)
